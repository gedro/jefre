...
export default function CandidateList({ classes, appContext, onAppContextChanged, history }) {
  ...
  useEffect(() => {
    fetchCandidates().catch((err) => {
      toast.error("Error fetching suggested candidates", err);
    });
    if(jobId) {
  ...
  const fetchCandidates = async () => {
    try {
      ...
      const response = await appContext.api.get(`/jobs/${jobId}/candidates`);
      ...
      {loading ? ( <Loader /> ) : (
        <div className={classes.re_recruiter}>
          ...
          <div className={classes.re_recruiter_candidates_container}>
            {candidates.map(({candidate, score, occupationCoverage, skillCoverage}) => (
